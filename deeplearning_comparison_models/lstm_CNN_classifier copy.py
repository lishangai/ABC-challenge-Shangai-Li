...
class CNNLSTMClassifier(nn.Module):
    """CNN+LSTM 分类模型"""
    def __init__(self, input_size=3, cnn_out_channels=256, kernel_size=3, lstm_hidden_size=128, num_lstm_layers=3, num_classes=10, dropout=0.2):
        ...
        self.bn1 = nn.BatchNorm1d(cnn_out_channels)  # 添加Batch Normalization
        ...
        self.bn2 = nn.BatchNorm1d(cnn_out_channels * 2)  # 添加Batch Normalization
        self.pool = nn.MaxPool1d(kernel_size=2, stride=2)
        
        # LSTM 部分 
        self.lstm = nn.LSTM(
            input_size=cnn_out_channels * 2,  # CNN 输出的通道数作为 LSTM 的输入维度
            hidden_size=lstm_hidden_size,     # LSTM 隐藏层大小
            num_layers=num_lstm_layers,       # LSTM 层数
            batch_first=True,                  # 设置为 batch_first 以适应输入数据的形状
            dropout=dropout,
        ...
        self.fc = nn.Sequential(
            nn.Linear(lstm_hidden_size * 2, 64),  # 双向 LSTM 会产生 2 * lstm_hidden_size 维度的输出
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, x):
        # CNN 部分
        x = x.transpose(1, 2)  # 转置，使数据形状符合卷积层输入要求 (batch_size, channels, seq_len)
        x = self.conv1(x)      # 卷积操作
        ...
        x = self.pool(x)       # 池化操作
        x = x.transpose(1, 2)  # 转置，使数据形状符合 LSTM 输入要求 (batch_size, seq_len, channels)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=50, device='cuda'):
    ...
    for epoch in range(num_epochs):
        ...
        print('-' * 60)
    print(f'Bestmodel with validation accuracy: {best_val_acc:.2f}%')
